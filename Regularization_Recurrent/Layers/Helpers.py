...
class IrisData:
    def __init__(self, batch_size, random=True):
        ...
        # Use OneHotEncoder in a way that's compatible across scikit-learn versions
        encoder = OneHotEncoder()
        labels = encoder.fit_transform(self._data.target.reshape(-1, 1))
        # fit_transform may return a sparse matrix depending on sklearn version
        if hasattr(labels, 'toarray'):
            labels = labels.toarray()
        self._label_tensor = labels
