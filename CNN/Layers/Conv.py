...
class Conv(Base):
    ...
    def backward(self, error_tensor):
        k_size = tuple(self.kernel_shape[1:])
        self.gradient_bias = np.sum(error_tensor, axis=(0, 2) if self.is_1d else (0, 2, 3)).flatten()
        self.gradient_weights = np.zeros_like(self.weights)
        
        if self.is_1d:
            in_len, out_len = self._input_shape[2], error_tensor.shape[2]
            _, (ps, pe) = self._calc_pad(in_len, self.stride[0], k_size[0])
            ...
            for b in range(error_tensor.shape[0]):
                for k in range(self.num_kernels):
                    for l in range(out_len):
                        s = l * self.stride[0]
                        self.gradient_weights[k] += error_tensor[b, k, l] * padded[b, :, s:s+k_size[0]]
                        grad_pad[b, :, s:s+k_size[0]] += error_tensor[b, k, l] * self.weights[k]
            grad = self._unpad(grad_pad, ps, pe, 2)
        else:
            in_h, in_w = self._input_shape[2:4]
            out_h, out_w = error_tensor.shape[2:4]
            _, (phs, phe) = self._calc_pad(in_h, self.stride[0], k_size[0])
            _, (pws, pwe) = self._calc_pad(in_w, self.stride[1], k_size[1])
            ...
            for b in range(error_tensor.shape[0]):
                for k in range(self.num_kernels):
                    for h in range(out_h):
                        for w in range(out_w):
                            hs, ws = h * self.stride[0], w * self.stride[1]
                            self.gradient_weights[k] += error_tensor[b, k, h, w] * padded[b, :, hs:hs+k_size[0], ws:ws+k_size[1]]
                            grad_pad[b, :, hs:hs+k_size[0], ws:ws+k_size[1]] += error_tensor[b, k, h, w] * self.weights[k]
            grad = self._unpad(self._unpad(grad_pad, phs, phe, 2), pws, pwe, 3)
        
        if getattr(self, '_opt_w', None):
            self.weights = self._opt_w.calculate_update(self.weights, self.gradient_weights)
        if getattr(self, '_opt_b', None):
